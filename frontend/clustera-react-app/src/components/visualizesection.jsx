...
function VisualizationSection({ summarizedDocuments, noOfClusters, clustersPredicted, topicsGenerated, clustersGenerated, tsneParameters, documentTopicDistributionThreshold, topicsGeneratedLabel }) {
    ...
    return (
        isComponentLoading ? (
            ...
            <div className="flex-col px-2 py-1 flex w-full mx-3 border rounded-lg drop-shadow-lg max-w-[1580px]">
                <div className="flex flex-row justify-start ml-12 mt-2">
                    {
                        showScatterplotChart ? (
                            <button disabled={true} onClick={() => toggleChartType()} className="bg-blue-700 hover:bg-blue-700 text-white font-semibold py-1 px-4 rounded shadow">Scatterplot</button>
                        ) : (
                            <button onClick={() => toggleChartType()} className="bg-blue-500 hover:bg-blue-700 text-white font-semibold py-1 px-4 rounded shadow">Scatterplot</button>
                        )
                    }
                    {
                        showWordcloudCharts ? (
                            <button disabled={true} onClick={() => toggleChartType()} className="bg-blue-700 hover:bg-blue-700 text-white font-semibold py-1 px-4 rounded shadow ml-8">Wordcloud</button>
                        ) : (
                            <button onClick={() => toggleChartType()} className="bg-blue-500 hover:bg-blue-700 text-white font-semibold py-1 px-4 rounded shadow ml-8">Wordcloud</button>
                        )
                    }
                </div>
                ...
                {showScatterplotChart ? (
                    ...
                    />
                ) : <div className="flex flex-row flex-wrap justify-start mt-16 max-w-[1580px]">
                    {
